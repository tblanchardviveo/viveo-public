...
const S = {
  ...
  err:  { color:'#F87171', fontFamily:'Raleway,sans-serif', fontSize:13, fontStyle:'italic' },
...
export default function AnalyseFiscale({ prog, fiscalites, prixMin }) {
  ...
  useEffect(() => {
    ...
    if (!key) { setErr('Clé API non configurée (VITE_ANTHROPIC_KEY)'); setLoading(false); return }
    ...
    .then(r=>r.json())
    .then(d=>{ setTexte(d.content?.[0]?.text||''); setLoading(false) })
    .catch(()=>{ setErr('Analyse temporairement indisponible'); setLoading(false) })
  }, [prog?.id])
