...
export default function FicheProgramme() {
  ...
  if (!programme) return <div style={{ maxWidth: 900, margin: '0 auto', padding: '80px 40px', textAlign: 'center' }}><p style={{ fontFamily: '"Playfair Display", serif', fontSize: 24,  }}>Programme introuvable.</p><p style={{ color: 'var(--bronze)', fontSize: 14, marginTop: 16, display: 'inline-block' }}><Link to="/">&larr; Retour</Link></p></div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', maxWidth: 1100, margin: '0 auto', padding: '60px 32px', gap: 48, flexWrap: 'wrap' }}>
          ...
          {p.dispositifs_fiscaux && p.dispositifs_fiscaux.length > 0 && (
            <div style={{ marginTop: 40 }}><Eyebrow>DISPOSITIFS FISCAUX</Eyebrow>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                {p.dispositifs_fiscaux.map(d => <Link key={d} to={dispoLinks[d]} style={{ display: 'inline-block', padding: '10px 20px', border: '1.5px solid var(--bronze)', borderRadius: 50, color: 'var(--bronze)', fontSize: 13, fontWeight: 600, textDecoration: 'none', fontFamily: '"Raleway", sans-serif' }}>{d} &rarr;</Link>)}
              </div>
            </div>)}
          {(p.latitude && p.longitude) && (
            <div style={{ marginTop: 40 }}><Eyebrow>LOCALISATION</Eyebrow>
                            <iframe title="carte" src={`https://www.openstreetmap.org/export/embed.html?bbox=${p.longitude-0.01},${p.latitude-0.01},${p.longitude+0.01},${p.latitude+0.01}&layer=mapnik&marker=${p.latitude},${p.longitude}`} style={{ width: '100%', height: 300, border: '1.5px solid #E5E3E0', borderRadius: 12 }} />
            </div>)}
        ...
        {/* DONNÉES COMMUNE */}       {p.afficher_metadonnees && (         <div style={{ flex: '1 1 100%', marginTop: 48, borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 48 }}>           <Eyebrow>LA COMMUNE</Eyebrow>           {p.description_ville && <p style={{ fontSize: 15, lineHeight: 1.8, color: 'rgba(255,255,255,0.85)', fontFamily: '"Raleway", sans-serif', marginBottom: 32 }}>{p.description_ville}</p>}           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>             {p.population && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{new Intl.NumberFormat('fr-FR').format(p.population)}</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Habitants</p></div>}             {p.revenu_median && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{new Intl.NumberFormat('fr-FR').format(p.revenu_median)} €</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Revenu médian</p></div>}             {p.taux_chomage && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{p.taux_chomage}%</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Taux de chômage</p></div>}             {p.part_proprietaires && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{p.part_proprietaires}%</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Propriétaires</p></div>}             {p.prix_m2_ancien && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{new Intl.NumberFormat('fr-FR').format(p.prix_m2_ancien)} €/m²</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Prix ancien</p></div>}             {p.tension_locative && <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, textAlign: 'center' }}><p style={{ fontSize: 22, fontWeight: 700, color: 'var(--bronze)', fontFamily: '"Playfair Display", serif', margin: '0 0 4px' }}>{p.tension_locative}</p><p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: '"Raleway", sans-serif', margin: 0 }}>Tension locative</p></div>}           </div>         </div>       )}       {/* SIDEBAR */}
