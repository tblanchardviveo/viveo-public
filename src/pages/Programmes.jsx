...
import FiltresProgrammes from '../components/programmes/FiltresProgrammes'
import CartePNeufs from '../components/programmes-neufs/CartePNeufs'

const INIT = { ville:'', region:'', promoteur:'', pieces:'', prixMin:'', prixMax:'', zone:'', fiscas:[] }
const S = {
...
export default function Programmes() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setLots(ld)
      setPromoteurs(promots || [])
      setLoading(false)
    })
  }, [])

  const villes  = useMemo(() => [...new Set(programmes.map(p=>p.ville).filter(Boolean))].sort(), [programmes])
  ...
  const filtered = useMemo(() => programmes.filter(p => {
    ...
    if (filtres.promoteur && p.promoteur_id !== filtres.promoteur) return false
    if (filtres.zone && p.zone_fiscale !== filtres.zone) return false
    if (filtres.prixMin && p.prixMin && p.prixMin < Number(filtres.prixMin)) return false
  ...
  return (
    <div>
      <div style={S.hero}>
        <h1 style={S.h1}>Programmes immobiliers neufs</h1>
        <p style={S.sub}>Catalogue Nexity — investissement et résidence principale</p>
      </div>
      <FiltresProgrammes filtres={filtres} onChange={setFiltres}
        villes={villes} regions={regions} promoteurs={promoteurs} />
      <div style={S.main}>
        <div style={S.count}>
        ...
        {!loading && filtered.length === 0
          ...
          : <div style={S.grid}>{filtered.map(p=><CartePNeufs key={p.id} prog={p} onSelect={()=>navigate(`/programmes/${p.id}`)}/>)}</div>}
      </div>
    </div>
  )
}
