...
const SimulateurHub = lazy(() => import('./pages/Simulateurs'))
  ...
  return (
    <>
      <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--navy-deep)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.06)', height: 72 }}>
        <div style={{ width: '100%', padding: '0 32px', display: 'flex', alignItems: 'center', height: '100%', boxSizing: 'border-box', gap: 0 }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'baseline', gap: 0, textDecoration: 'none' }}>
            <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 20, color: '#fff', fontWeight: 400 }}>VIVEO</span>
            <span style={{ color: 'var(--bronze)', margin: '0 4px', fontSize: 16 }}>{"·"}</span>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 20, flex: 1, marginLeft: 24 }}>
            ...
            <Link to="/approche-viveo" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>Notre Approche</Link><Link to="/financement" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>Financement</Link><Link to="/parrainage" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>Parrainage</Link><Link to="/simulateurs" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>Simulateurs</Link><Link to="/international" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>International</Link><Link to="/blog" style={linkStyle} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#C4976A'} onMouseLeave={e => e.target.style.color='rgba(255,255,255,0.6)'}>Blog</Link><Link to="/diagnostic" style={{ ...linkStyle, color: '#C4976A', fontWeight: 600 }} className="nav-hide-mobile" onMouseEnter={e => e.target.style.color='#fff'} onMouseLeave={e => e.target.style.color='#C4976A'}>Diagnostic gratuit</Link>
            <Link to="/souscrire" className="nav-hide-mobile" style={{ marginLeft: 'auto', padding: '7px 12px', borderRadius: 20, fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', border: '1px solid rgba(196,151,106,0.5)', color: '#C4976A', background: 'transparent', textDecoration: 'none', whiteSpace: 'nowrap' }}>Essai gratuit 14j</Link>
            <a href="https://viveo-patrimoine.fr/partenaire" className="nav-hide-mobile" style={{ padding: '7px 12px', borderRadius: 20, fontSize: 10, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', border: '1px solid #A67C52', color: '#A67C52', background: 'transparent', textDecoration: 'none', whiteSpace: 'nowrap' }}>Espace partenaire</a>
            <a href={'/rdv-decouverte'} target="_blank" rel="noopener noreferrer" className="nav-hide-mobile" style={{ background: 'linear-gradient(135deg, #A67C52 0%, #C4976A 100%)', color: '#fff', borderRadius: 2, padding: '8px 14px', fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: "'Raleway', sans-serif", textDecoration: 'none', transition: 'all 0.3s var(--ease)' }}>{"Prendre RDV →"}</a>
      ...
      {isMobile && menuOpen && (
        <div style={{ position: 'fixed', top: 72, left: 0, right: 0, background: 'var(--navy-deep)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '16px 24px', zIndex: 99, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Link to="/international" onClick={() => setMenuOpen(false)} style={{ ...linkStyle, fontSize: 13, color: '#fff' }}>International</Link><Link to="/blog" onClick={() => setMenuOpen(false)} style={{ ...linkStyle, fontSize: 13, color: '#fff' }}>Blog</Link><Link to="/parrainage" onClick={() => setMenuOpen(false)} style={{ ...linkStyle, fontSize: 13, color: '#fff' }}>Parrainage</Link><Link to="/simulateurs" onClick={() => setMenuOpen(false)} style={{ ...linkStyle, fontSize: 13, color: '#fff' }}>Simulateurs</Link><Link to="/diagnostic" onClick={() => setMenuOpen(false)} style={{ ...linkStyle, fontSize: 13, color: '#C4976A', fontWeight: 600 }}>Diagnostic gratuit</Link>
        </div>
      )}
    </>
  )
